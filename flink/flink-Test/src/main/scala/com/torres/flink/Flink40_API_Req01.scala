...
class KeyedProcessWaterSensor extends KeyedProcessFunction[String, WaterSensor, String] {
    private var currentVC = 0.0
    ...
    override def processElement(value: WaterSensor, ctx: KeyedProcessFunction[String, WaterSensor, String]#Context, out: Collector[String]): Unit = {
        ...
        if (value.vc > currentVC) {
            if (alarmTime == 0L) {
                alarmTime = value.ts*1000 + 5000
